...
class ReRaLSTM:
    def __init__(self, data_path, market_name, tickers_fname, relation_name,
                 emb_fname, parameters, steps=1, epochs=50, batch_size=None, flat=False, in_pro=False,
                 channel_size=5,st_layer=1,time_kernel_size=3,seed=123456789):
        ...
        # print(np.sum(self.mask_data==1)/(self.mask_data.shape[0]*self.mask_data.shape[1]));exit()
        # relation data
    ...
    def get_batch_stgcn(self, offset=None):
        if offset is None:
            offset = random.randrange(0, self.valid_index)
        seq_len = self.parameters['seq']
        mask_batch = self.mask_data[:, offset: offset + seq_len + self.steps]
        mask_batch = np.min(mask_batch, axis=1)
        feature = self.eod_data[:,offset: offset + seq_len,:]
        return feature, \
               np.expand_dims(mask_batch, axis=1), \
               np.expand_dims(
                   self.price_data[:, offset + seq_len - 1], axis=1
    ...
    def st_gcn(self,input, in_channels, out_channels, time_kernel_size, stride, layer,
               residual=True, share_weight=True):
        # KGCN
        # matrix transform: N x T x C

        # feature = tf.layers.conv1d(input,kernel_size=1, filters=out_channels,padding='SAME',strides=1,
        #                        activation=self.activation)
        feature = input
        rel_weight = tf.layers.dense(self.relation, units=1,
                                     activation=self.activation)
        if share_weight:
            time_outputs = []
            for time in range(self.parameters['seq']):
                ...
                if self.inner_prod:
                    ...
                    with tf.variable_scope("layer_{}".format(layer), reuse=tf.AUTO_REUSE):
                        head_weight = tf.layers.dense(feature_time, units=1,
                                                      activation=self.activation)
                        tail_weight = tf.layers.dense(feature_time, units=1,
                                                      activation=self.activation)
                    weight = tf.add(
                        tf.add(
                ...
                weight_masked = tf.nn.softmax(tf.add(self.rel_mask, weight), dim=0)
                outputs_proped = tf.matmul(weight_masked, feature_time)
                time_outputs.append(outputs_proped)
        else:
            time_outputs = []
            for time in range(self.parameters['seq']):
                feature_time = feature[:,time,:]
        ...
        outputs_proped = tf.stack(time_outputs,axis=1)
        # TCN
        output = tf.layers.conv1d(outputs_proped,kernel_size=time_kernel_size, filters=out_channels,
        ...
        if not residual:
            ...
            res = tf.layers.conv1d(input,kernel_size=1, filters=out_channels, strides=stride,padding='SAME')
            output = res+output
    ...
    def train(self):
        ...
        feature = tf.placeholder(tf.float32,
                                 [self.batch_size, self.parameters['seq'], self.channel_size])
        ...
        output = self.st_gcn(feature, in_channels=self.channel_size, out_channels=64, layer=1,
                             time_kernel_size=self.time_kernel_size, stride=1)
        for layer in range(2,self.st_layer+1):
            output = self.st_gcn(output,in_channels=64, out_channels=64,layer=layer,
                             time_kernel_size=self.time_kernel_size, stride=1)
        # time dimension average pooling
        output = tf.reduce_mean(output,axis=1)
        # One hidden layer
        prediction = tf.layers.dense(
            output, units=1, activation=self.activation, name='reg_fc',
        ...
        for i in range(self.epochs):
            ...
            for j in range(self.valid_index - self.parameters['seq'] -
                                   self.steps + 1):
                emb_batch, mask_batch, price_batch, gt_batch = self.get_batch_stgcn(
                    batch_offsets[j])
                feed_dict = {
                    feature: emb_batch,
                    mask: mask_batch,
            ...
            if cur_valid_perf['btl'] >= \
                ...
                print('Now best test btl:', best_test_perf['btl'])
            t4 = time()
            print('epoch:', i, ('time: %.4f ' % (t4 - t1)))
        print('\nBest Valid performance:', best_valid_perf)
        print('\tBest Test performance:', best_test_perf)
        logging.info('\nBest Valid performance:'+str(best_valid_perf))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', default=3,help='kernel size of time frames')
    ...
    logging.basicConfig(filename="st_"+args.m+"_"+str(args.seed), level=logging.INFO)
    ...
    RR_LSTM = ReRaLSTM(
        ...
        steps=1, epochs=50, batch_size=None,
        in_pro=args.inner_prod,
        time_kernel_size=args.k,
        seed=args.seed
    )
